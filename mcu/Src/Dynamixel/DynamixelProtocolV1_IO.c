...
)
{
    ...
    if(numArgs <= 3){
        ...
        uint8_t arr[TX_PACKET_SIZE];
        arr[0] = 0xFF;
        arr[1] = 0xFF;
        arr[2] = hdynamixel -> _ID;
        arr[3] = 2 + numArgs;
        arr[4] = INST_WRITE_DATA;
        for(uint8_t i = 0; i < numArgs; i ++){
            arr[5 + i] = args[i];
        }

        // Checksum
        arr[4 + numArgs + 1] = Dynamixel_ComputeChecksum(arr, 4 + numArgs + 2);

        // Transmit
        Dynamixel_GenericTransmit(hdynamixel, arr, 4 + numArgs + 2);
    }
}
...
)
{
    ...
    arr[4] = INST_READ_DATA; // READ DATA instruction
    arr[5] = readAddr; // Write address for register
    arr[6] = readLength; // Number of bytes to be read from motor
    arr[7] = Dynamixel_ComputeChecksum(arr, 8);
    ...
    if(!Dynamixel_GenericTransmit(hdynamixel, arr, 8)){
        hdynamixel -> _lastReadIsValid = false;
        return -1;
    }

    // Receive requested data
    if(!Dynamixel_GenericReceive(hdynamixel, arr, rxPacketSize)){
        hdynamixel -> _lastReadIsValid = false;
        return -1;
    }

    // Check data integrity and place this flag in a field the application can read
    recvChecksum = arr[rxPacketSize - 1];
    computedChecksum = Dynamixel_ComputeChecksum(arr, rxPacketSize);
...
)
{
    ...
    arr[0] = 0xFF;
    arr[1] = 0xFF;
    arr[2] = hdynamixel -> _ID == BROADCAST_ID ? 0 : hdynamixel -> _ID;
    arr[3] = arrSize - 4;
    arr[4] = INST_REG_WRITE;
    arr[5] = writeAddr;
    arr[7] = (
            (arrSize == 8) ?
            Dynamixel_ComputeChecksum(arr, arrSize) :
            param2
    );
    if(arrSize == 9){
        arr[8] = Dynamixel_ComputeChecksum(arr, arrSize);
    }

    // Transmit
    Dynamixel_GenericTransmit(hdynamixel, arr, arrSize);
}
...
void Dynamixel_Reset(Dynamixel_HandleTypeDef* hdynamixel){
    ...
    arr[4] = INST_RESET; // Reset instruction
    arr[5] = Dynamixel_ComputeChecksum(arr, 6);
